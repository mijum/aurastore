...
export function AdminLoginPage() {
  ...
  return <div className="min-h-screen bg-slate-950 text-white grid lg:grid-cols-2">
    ...
    <div className="flex items-center justify-center p-6 sm:p-12"><form onSubmit={submit} className="w-full max-w-md"><div className="lg:hidden flex items-center gap-3 mb-12"><span className="w-11 h-11 rounded-2xl bg-indigo-600 grid place-items-center"><Store /></span><span className="text-2xl font-black">AuraStore</span></div><p className="text-indigo-400 text-xs font-black uppercase tracking-[.2em]">Admin access</p><h2 className="text-4xl font-black mt-3">Welcome back</h2><p className="text-slate-400 mt-2 mb-8">Sign in with your administrator username/email and password.</p><label className="text-xs font-bold text-slate-300">Username or Email</label><input type="text" required value={email} onChange={(e) => setEmail(e.target.value)} className="mt-2 mb-5 w-full px-4 py-3.5 rounded-xl bg-white/5 border border-white/10 focus:border-indigo-500 outline-none" placeholder="admin or admin@aurastore.com" /><label className="text-xs font-bold text-slate-300">Password</label><div className="relative mt-2"><input type={show ? 'text' : 'password'} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-4 py-3.5 pr-12 rounded-xl bg-white/5 border border-white/10 focus:border-indigo-500 outline-none" placeholder="••••••••" /><button type="button" onClick={() => setShow(!show)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500">{show ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}</button></div>{error && <p className="mt-4 p-3 rounded-xl bg-rose-500/10 border border-rose-500/20 text-rose-300 text-sm">{error}</p>}<button disabled={busy} className="mt-6 w-full py-4 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-60 font-black flex items-center justify-center gap-2 shadow-xl shadow-indigo-950"><LockKeyhole className="w-4 h-4" />{busy ? 'Signing in…' : 'Sign in securely'}<ArrowRight className="w-4 h-4" /></button></form></div>
  </div>;
}
